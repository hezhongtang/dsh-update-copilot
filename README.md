...
| 🖥 **Web surfaces** | On Web hosts, a sidebar trigger beside Settings hydrates its badge on mount and retries after the startup scan; background scans run at startup and every 30 minutes. The compact popup shows behind rows first and folds up-to-date rows; aggregate-managed children stay with their parent in the same behind or up-to-date section. The full page lives in Settings → Update Copilot. Plugins merge into one package row with per-profile versions; aggregate-managed children expand beneath their parent as read-only rows. **Update** and **Update all** target only explicit eligible profiles. Updates stream live progress over SSE (resolving / downloading / retrying / stash / pull / restore phases) straight into a per-row progress bar |
