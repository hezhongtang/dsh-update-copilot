...
| 🛡 **Update guardrails** | Same-origin POST + explicit `confirm`, strict target allowlist, single-flight lock, 5-minute timeout; a preflight **hard gate** refuses updates the target dsh demonstrably breaks (missing named exports) until an explicit force; npm/github specs run only through the official `dsh plugin` CLI, `link:` checkouts update via git pull in their own directory (auto-stash → pull → restore; conflicts are always handed back for manual handling), while `file:` installs and official `@deepseek-ai/*` packages remain refused; **empty husk publishes** (frozen `latest` / no entry / no `dsh.bundle` / tiny tarball) are skipped when resolving the update target and refused for install |
