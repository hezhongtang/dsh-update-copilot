 ...
 * than the tag. Concurrent callers share one doc fetch.
 */
async function npmNewest(name) {
  ...
  const promise = (async () => {
    try {
      ...
      return { newest, distLatest: doc?.['dist-tags']?.latest ?? null }
    } catch {
      return { newest: null, distLatest: null }
    }
  })()
...
/** Local vs origin HEAD for a link: checkout (read-only git queries). */
...
export async function scanCore(force = false) {
  ...
  const rows = await Promise.all(names.map(async (name) => {
    ...
    const { newest } = await npmNewest(name)
    return {
      name,
  ...
  const rows = await Promise.all(Object.entries(deps).map(async ([name, spec]) => {
    ...
    if (kind === 'linked' || kind === 'file') {
      ...
      if (kind === 'linked' && existsSync(join(dir, '.git'))) {
        const { local, upstream } = await linkedGitState(dir)
        return {
          ...base,
          dir,
          current: local,
    ...
    const { newest, distLatest } = await npmNewest(name)
    return {
      ...base,
