...
export async function scanAll(force = false) {
  ...
  // the target) dsh host — disk-only reads; any failure degrades to no
  // warnings instead of failing the scan.
  try {
    const versions = hostVersionsFromCore(core, currentDshVersion())
    if (versions.length > 0) {
      for (const agg of plugins) {
        const profileRow = agg.profiles.find((p) => p.classification !== 'official')
        if (profileRow === undefined) continue
        const dir = packageDirOf(profileDir(profileRow.profile), agg.name, profileRow.spec)
        const findings = peerWarningsForPackageDir(dir, versions)
        if (findings.length > 0) agg.peerWarnings = findings
      }
    }
  } catch { /* peer warnings never fail the version scan */ }
