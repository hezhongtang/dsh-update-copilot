 ...
 * plugins (manifest `dsh.profile.bundles`) plus visible dev checkouts, across
 * all channels. See pluginEntries() for the exact membership rule.
 */
export async function scanProfile(profile, force = false) {
  ...
  const rows = await Promise.all(pluginEntries(deps, bundles).map(async ([name, spec]) => {
    const kind = classifySpec(spec)
