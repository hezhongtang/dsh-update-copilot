...
export async function scanProfile(profile, force = false) {
  ...
  // Membership = mainline rule (bundle-declared + dev checkouts, all deps
  // when the manifest declares no bundles) ∪ verified patch-mounted children.
  // Official deps are always scanned too: report-only rows, never plugin rows.
  const mount = inferMountRelationships(profile, deps)
  const members = new Set(pluginEntries(deps, bundles).map(([name]) => name))
