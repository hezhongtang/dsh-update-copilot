...
export function profileDependencyMetadata(profile, deps) {
  const aggregateSpec = deps[WEB_UI_ALL]
  const managed = aggregateSpec === undefined
    ? new Set()
    : new Set(Object.keys(readJson(join(profileDir(profile), 'node_modules', WEB_UI_ALL, 'package.json'))?.dependencies ?? {}))
  ...
  return Object.entries(deps).map(([name, spec]) => {
    ...
    if (name === WEB_UI_ALL) return { name, spec, kind, classification: 'aggregate' }
    if (kind === 'linked' || kind === 'file') return { name, spec, kind, classification: 'local' }
    if (name === BETTER_SIDEBAR || managed.has(name)) {
      return { name, spec, kind, classification: 'aggregate-managed', managedBy: WEB_UI_ALL }
    }
...
export async function scanAll(force = false) {
  ...
  const reportedProfiles = profiles.filter((profile) => profile.plugins.length > 0 || profile.official.length > 0 || profile.managed.length > 0)
  const result = {
