...
/** Aggregate parents inferred from active profile bundle patches. */
function aggregateOwnership(profile, deps) {
  ...
  return { parents: new Set(parents.map((parent) => parent.name)), owners: new Map([...owners].map(([child, parent]) => [child, parent.name])) }
}
