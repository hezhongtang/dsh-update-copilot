...
export function inferMountRelationships(profile, deps) {
  ...
  for (const parent of parents) {
    for (const child of [...parent.children].sort()) {
      if (deps[child] !== undefined) relations.push({ parent: parent.name, child, evidence: 'patch-insert+production-dependency' })
    }
  }
  const selected = new Map()
  for (const relation of relations) if (!selected.has(relation.child)) selected.set(relation.child, relation.parent)
  return { parents: new Set(parents.map((parent) => parent.name)), relations, selected }
}

/** Ownership metadata for every direct dependency in one profile. */
export function profileDependencyMetadata(profile, deps) {
  const { parents, selected } = inferMountRelationships(profile, deps)
  ...
  return Object.entries(deps).map(([name, spec]) => {
    ...
    return { name, spec, kind, classification: 'independent', ...(selected.has(name) ? { mountedBy: selected.get(name) } : {}) }
  })
}
...
export async function scanProfile(profile, force = false) {
  ...
  const relationships = inferMountRelationships(profile, deps).relations
  const rows = await Promise.all(profileDependencyMetadata(profile, deps).map(async ({ name, spec, kind, classification, mountedBy }) => {
