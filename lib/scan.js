...
import { summarizeAvailability, verdictAvailability, worstAvailability, packageDirOf } from './availability.js'
import { currentDshVersion, hostVersionsFromCore, peerWarningsForPackageDir } from './preflight.js'
import { looksEmptyPackage, newestHealthyVersion } from './empty-pkg.js'
 ...
 * structural `#path:/...` fragment verbatim. Pin fragments (`#<sha>` /
 * `#<branch>` — e.g. the spec a rollback install leaves behind) are dropped:
 * the lockfile's codeload URLs carry the pin in the URL path, never as a
 * fragment. Mirrors pinnedCommits().
 */
export function githubCommitKey(spec) {
  const m = GITHUB_SPEC.exec(spec)
  return m !== null ? `${m[1].toLowerCase()}${githubPathFragment(m[2]) ?? ''}` : null
}
 ...
 * than the tag. Empty husk publishes (no entry, no dsh.bundle, tiny tarball —
 * the frozen `latest=0.0.1-rc.1` family) are skipped so an update never
 * resolves to them. The already-fetched doc also yields the GitHub repository,
...
export async function npmNewest(name, force = false) {
  ...
  promise = (async () => {
    const result = await captureProbe(async () => {
      ...
      const { version: newest, skippedEmpty, meta } = newestHealthyVersion(doc?.versions ?? {}, semverCompare)
      const distLatest = doc?.['dist-tags']?.latest ?? null
      const distLatestMeta = distLatest !== null ? doc?.versions?.[distLatest] ?? null : null
      const publishedAt = typeof doc?.time?.[newest] === 'string' ? doc.time[newest] : null
      return {
        newest,
        newestMeta: meta,
        newestEmpty: newest !== null && looksEmptyPackage(meta),
        skippedEmpty,
        emptyLatest: distLatest !== null && looksEmptyPackage(distLatestMeta),
        distLatest,
        distLatestMeta,
        publishedAt,
        versions: Object.keys(doc?.versions ?? {}),
        repo: repoOf(doc?.repository),
        repoUrl: repoUrlOf(doc?.repository),
      }
    })
    if (!result.ok) {
      const failure = {
...
export async function scanProfile(profile, force = false) {
  ...
  const rows = await Promise.all(profileDependencyMetadata(profile, deps, mount)
    ...
    return {
      ...
      emptyLatest: meta.emptyLatest === true,
      skippedEmpty: Array.isArray(meta.skippedEmpty) ? meta.skippedEmpty : [],
      publishedAt: meta.publishedAt ?? null,
      updateAvailable: isNewer(meta.newest, current),
      ...(meta.emptyLatest === true
        ? { note: `latest dist-tag ${meta.distLatest} is an empty publish — update pins ${meta.newest}` }
        : {}),
      ...unreachableFields(meta),
    }
  }))
...
export async function scanAll(force = false) {
  ...
  // Peer-range warnings against the running (and, when the core is behind,
  // the target) dsh host — evaluated per installing profile (different
  // profiles may carry different specs/manifests), each finding tagged with
  // its profile. Disk-only reads; any failure degrades to no warnings.
  try {
    const versions = hostVersionsFromCore(core, currentDshVersion())
    if (versions.length > 0) {
      for (const agg of plugins) {
        const findings = []
        for (const profileRow of agg.profiles) {
          if (profileRow.classification === 'official') continue
          const dir = packageDirOf(profileDir(profileRow.profile), agg.name, profileRow.spec)
          for (const finding of peerWarningsForPackageDir(dir, versions)) {
            findings.push({ ...finding, profile: profileRow.profile })
          }
        }
        if (findings.length > 0) agg.peerWarnings = findings
      }
    }
  } catch (error) {
    recordOp('warn', 'scan:preflight', error instanceof Error ? error.message : String(error))
  }
