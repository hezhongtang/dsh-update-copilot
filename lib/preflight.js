...
import { profileDir, profilesRoot, readJson, recordOp } from './util.js'
...
function caretComparators(v) {
  if (isWildcard(v.major)) return [] // `^*` / `^x` — unconstrained
...
const ROLE_TEXT = { current: { zh: '当前', en: 'current' }, target: { zh: '目标', en: 'target' } }
 ...
 * outside the harness contract. Findings carry the evidence fields plus a
 * bilingual message in the house 'zh / en' single-string convention; the
 * client prefers its own i18n rendering, the agent tools and CLI print the
 * message as-is.
 * @param {{ name?: string|null, peerDependencies?: Record<string, string>, versions: Array<{ role: 'current'|'target', version: string|null }> }} args
 * @returns {Array<{ plugin: string|null, type: 'peer', specifier: string, range: string, against: string, version: string, message: string }>}
 */
export function peerRangeFindings({ name = null, peerDependencies, versions }) {
  ...
  for (const [specifier, range] of Object.entries(peerDependencies ?? {})) {
    ...
    for (const { role, version } of versions ?? []) {
      ...
      const roleText = ROLE_TEXT[role] ?? { zh: role, en: role }
      findings.push({
        plugin: name,
        type: 'peer',
        specifier,
        ...
        against: role,
        version,
        message: `${specifier} 声明 ${range}，不包含${roleText.zh} dsh ${version} / ${specifier} declares ${range}, which does not include ${roleText.en} dsh ${version}`,
      })
    }
  }
  return findings
...
/**
 * Breaking-change signals over the brief's already-fetched material —
...
export function breakingFindings(material = {}) {
  ...
  // Commits first — the brief lists them before the release notes too.
...
/**
 * The full preflight evaluation for one installed plugin, shared by the scan
 * attach, the brief, and the update executor: resolve the package directory
 * (link:/file: checkouts included), evaluate its peer ranges against the
 * core scan's current/target host versions, and degrade to no warnings —
 * logged — on any failure. `core` is the scanCore() result the caller
 * already has (passing it here keeps this module free of scan imports);
 * `dshVersion` overrides the on-disk current version when the caller knows
 * it more reliably.
 */
export async function peerWarningsForInstall({ profile, name, spec, core = null, dshVersion = null }) {
  try {
    const versions = hostVersionsFromCore(core, dshVersion ?? currentDshVersion())
