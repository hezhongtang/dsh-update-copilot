    ...
    for (const name of packages) {
      for (const snapshot of listSnapshots(profile, name)) {
        ...
        if (rollback === null) {
          ...
          // dsh may be the thing that will not boot — pnpm runs inside the
          // profile directory and reaches the same install directly.
          const dshCommand = `dsh plugin --profile ${snapshot.profile ?? profile} add ${rollback.target}`
          const pnpmCommand = `pnpm -C ${profileDir(snapshot.profile ?? profile)} add ${rollback.target}`
          stdout.write(`  ${zh ? '回滚' : 'rollback'}: ${dshCommand}\n`)
          stdout.write(`  ${zh ? '或（dsh 不可用时）' : 'or, when dsh itself is broken'}: ${pnpmCommand}\n`)
        }
