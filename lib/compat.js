...
function formatFinding(finding, zh) {
  ...
  const hostNote = finding.hostMissing
    ? (zh ? '（目标 host 包未找到）' : ' (host package not found)')
    : ''
  lines.push(zh
    ? `    ${finding.file} 从 ${finding.specifier} 导入 ${missing}，当前导出里没有${hostNote}`
    : `    ${finding.file} imports ${missing} from ${finding.specifier} — not exported${hostNote}`)
...
async function defaultLoadTargetExports(specifiers, version) {
  ...
  for (const spec of specifiers) {
    ...
    try {
      const packed = await execText('npm', ['pack', `${spec}@${version}`, '--pack-destination', dir], { timeoutMs: 60000, cwd: dir })
      if (packed === null) continue
      const tgzName = packed.split(/\r?\n/).filter(Boolean).at(-1)
      if (typeof tgzName !== 'string' || tgzName === '') continue
      const extracted = await execText('tar', ['-xzf', join(dir, tgzName), '-C', dir], { timeoutMs: 15000 })
      if (extracted === null && !existsSync(join(dir, 'package', 'package.json'))) continue
      if (!existsSync(join(dir, 'package', 'package.json'))) continue
      map[spec] = readHostExports(join(dir, 'package')).names
    } catch { /* degrade this specifier */ } finally {
