...
const targetPackCache = new TtlCache(GATHER_TTL_MS)

/** Cache wrapper over the npm-pack target pass: one 10-minute TTL shared by
 *  the scan's compat gather and the update gate, so an update click never
 *  re-packs what the scan just packed. (The cache supports a force bypass,
 *  which no caller currently uses — the gate's conservative reading means a
 *  stale pack can only delay a block, never create a false one.) */
async function loadTargetExportsCached(specifiers, version, force = false) {
  const key = `targetpack:${version}:${specifiers.join(',')}`
  const cached = targetPackCache.get(key, force)
  if (cached !== undefined) return cached
  const map = await defaultLoadTargetExports(specifiers, version)
  targetPackCache.set(key, map)
  return map
}

/**
 * Hard-evidence blocker findings for one plugin: the target dsh host line
 * lacks a named export the plugin statically imports — the failure that
 * bricks the whole plugin tree at boot. Deliberately the same conservative
 * reading as the scan's target pass: a failed pack, an unpacked host
 * package, or an independently versioned package reads as silence, never a
 * block (宁漏拦不错拦 — missed blocks degrade to warnings, false blocks
 * would lock users out of updates).
 */
export async function gatherTargetBlockers({
  profile = null,
  name,
  dir,
  targetVersion,
  force = false,
  loadTargetExports = loadTargetExportsCached,
}) {
  if (typeof targetVersion !== 'string' || targetVersion === '') return []
  if (typeof dir !== 'string' || dir === '' || isOfficialPackage(name)) return []
  const imports = collectPluginHostImports(dir)
  if (imports.length === 0) return []
  const specifiers = [...new Set(imports.map((row) => row.specifier))]
  let targetExports
  try {
    targetExports = await loadTargetExports(specifiers, targetVersion, force)
  } catch {
    return []
  }
  if (targetExports === null || typeof targetExports !== 'object' || Object.keys(targetExports).length === 0) return []
  const findings = diffHostImports(imports, targetExports, { against: 'target', hostVersion: targetVersion })
  const entryId = cordisEntryId(dir)
  const disablePatch = entryId !== null ? disablePatchSnippet(entryId) : null
  const removeCommands = profile !== null ? [removeCommand(profile, name)] : []
  return findings.map((finding) => ({
    ...finding,
    entryId,
    ...(disablePatch !== null ? { disablePatch } : {}),
    ...(removeCommands.length > 0 ? { removeCommands } : {}),
  }))
}
