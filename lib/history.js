 ...
 * (anything else would either make pnpm install a different package or
 * silently migrate the dependency across channels). Returns
...
export function validateRollbackTarget(name, spec, target) {
  ...
  const specRepo = /^(?:github:)?([A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+?)(?:#.*)?$/.exec(specText)
  if (specRepo !== null) {
    ...
    return { target, targetVersion: null, repoKey: `${gh[1].toLowerCase()}${gh[2]}` }
  }
