...
export async function registerCopilotTools(tools, ctx = null) {
  ...
  disposers.push(tools.register(defineTool({
    ...
    description: 'Scan the DeepSeek Harness install for available updates: the dsh core + shipped bundles, and every plugin across all profiles, merged package-centric (a package installed in several profiles appears once with per-profile channels, ownership, versions, and eligible update profiles). Also reports third-party plugins whose named imports of @deepseek-ai/* are missing from the current (and, when the core is behind, the target) DSH host packages — the class of error that can fail the whole plugin tree at boot. Read-only. Call when the user asks "有没有更新/什么落后了/check for updates".',
    parameters: {
