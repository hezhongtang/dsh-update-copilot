  ...
  disposers.push(tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (args.profile !== undefined && args.profile !== '') {
        return updatePlugin(args.profile, args.name, { reload }, { source: args.source, ...(args.target !== undefined ? { target: args.target } : {}) })
      }
      return updatePluginAll(args.name, { reload }, { source: args.source, ...(args.target !== undefined ? { target: args.target } : {}) })
    },
