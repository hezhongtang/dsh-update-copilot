 ...
 * tight: scan (what is behind, package-centric across profiles), brief (what
 * an update contains + risk), preflight (the gate decision for one package,
 * read-only) and update (execute one confirmed update). The tools are the
 * agent-facing half of the "detect → decide → act" flow; the GUI mirrors the
 * same routes.
 */
...
export async function registerCopilotTools(tools, ctx = null) {
  ...
  const { updatePlugin, updatePluginAll, isUpdateRunning, UPDATE_TOOL_TIMEOUT_MS, evaluatePreflight } = await import('./update.js')
  ...
  disposers.push(tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (args.profile !== undefined && args.profile !== '') {
        return updatePlugin(args.profile, args.name, { reload }, { source: args.source, ...(args.target !== undefined ? { target: args.target } : {}), ...(args.force === true ? { force: true } : {}) })
      }
      return updatePluginAll(args.name, { reload }, { source: args.source, ...(args.target !== undefined ? { target: args.target } : {}), ...(args.force === true ? { force: true } : {}) })
    },
  ...
  disposers.push(tools.register(defineTool({
    ...
    description: 'Read-only preflight check for one installed package: the decision the update gate would make, WITHOUT updating anything. Reports the overall decision (ok / warning / blocked) with per-profile items — blockers (named imports the target dsh host no longer exports, the class of failure that bricks the whole plugin tree at boot), peer-range warnings against the current and target dsh, and breaking-change signals from release notes and commit subjects. Without a `profile`, every profile that has the package is evaluated; `target` overrides the dsh version line the evidence is gathered against (default: the newest published dsh when the core is behind). Use it to answer "will updating X break anything?" before calling update_copilot_update.',
    parameters: {
