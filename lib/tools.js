...
export async function registerCopilotTools(tools) {
  ...
  const { updatePlugin, isUpdateRunning } = await import('./update.js')
  ...
  disposers.push(tools.register(defineTool({
    ...
    description: 'Execute one plugin update through the official dsh plugin CLI, after the user explicitly confirmed it. Blocked for linked/file installs and @deepseek-ai/* official packages; the dsh core is report-only. Always run update_copilot_brief first and present the risk to the user.',
    parameters: {
    ...
    async execute(args) {
      ...
      return updatePlugin(args.profile, args.name)
    },
    timeoutMs: 330000,
  })))
