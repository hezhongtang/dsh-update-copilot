...
export async function registerCopilotTools(tools, ctx = null) {
  ...
  disposers.push(tools.register(defineTool({
    ...
    description: 'Scan the DeepSeek Harness install for available updates: the dsh core + shipped bundles, and every plugin across all profiles, merged package-centric (a package installed in several profiles appears once; the update command is identical for every profile). Read-only. Call when the user asks "有没有更新/什么落后了/check for updates".',
    parameters: {
  ...
  disposers.push(tools.register(defineTool({
    ...
    description: 'Build update highlights for one outdated package: semver distance, risk level, changelog material (versions, GitHub commits/release notes, or local git log), and a recommendation. When a `profile` is given, the brief covers that profile only; without it, every profile that has the package installed contributes a brief. Read-only. Use it to advise the user before updating.',
    parameters: {
  ...
  disposers.push(tools.register(defineTool({
    ...
    description: 'Execute one confirmed plugin update. Without a `profile`, the package is updated in every profile that has it installed (the update command is identical for all profiles); with a `profile`, only that one. npm/github specs run through the official dsh plugin CLI, with automatic retries (3 total attempts, 1s/3s backoff); link: local checkouts update through git pull in their own directory (auto-stash local changes → pull → restore; merge conflicts or failed restores are reported for manual handling). For link: checkouts, source="remote" switches the profile dependency to the published npm version (or a github: spec when the package is not on npm) — this breaks the local link and needs explicit user approval. The result reports attempts and the last output per profile. Blocked for file: installs and @deepseek-ai/* official packages; the dsh core is report-only. A successful update is hot-reloaded in the running process when phase-1 conditions hold; check hotReloaded/requiresRestart in the result. Always run update_copilot_brief first and present the risk to the user.',
    parameters: {
      ...
      profile: { type: 'string', description: 'Optional: update only this profile (e.g. "web"); default updates every profile that has the package installed.' },
