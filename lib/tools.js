...
export async function registerCopilotTools(tools, ctx = null) {
  ...
  disposers.push(tools.register(defineTool({
    name: 'update_copilot_brief',
     description: 'Build update highlights for one outdated package: semver distance, risk level, changelog material (versions, GitHub commits/release notes, or local git log), and a recommendation. When a `profile` is given, the brief covers that profile only; without it, every visible profile row that has the package installed contributes a brief, including mixed npm, GitHub, and local channels. Read-only. Use it to advise the user before updating.',
    parameters: {
