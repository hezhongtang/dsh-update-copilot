...
export async function registerCopilotTools(tools, ctx = null) {
  ...
  disposers.push(tools.register(defineTool({
    ...
    description: 'Read-only preflight check for one installed package: the decision the update gate would make, WITHOUT updating anything. Reports the overall decision (ok / warning / blocked) with per-profile items — hard blockers (named imports the target dsh host no longer exports, missing upgrade-card corridor edges, session-format boundaries), peer-range warnings against the current and target dsh, host-API capability risks, and breaking-change signals. Findings carry `layer` (link-time / mount-time / run-time / storage / peer / breaking-card) and `layerFindings`. Without a `profile`, every profile that has the package is evaluated; `target` overrides the dsh version line the evidence is gathered against (default: the newest published dsh when the core is behind). Use it to answer "will updating X break anything?" before calling update_copilot_update.',
    parameters: {
