 ...
 * same outcome object the agent tool returns).
 */
  ...
  disposers.push(host.webServer.register({
    ...
    handler: async (request, response) => {
      ...
      try {
        ...
        const { updatePlugin, subscribeProgress } = await import('./update.js')
        ...
        const outcome = await updatePlugin(profile, name, { reload }, { source })
