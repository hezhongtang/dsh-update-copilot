...
/**
 * Whole-profile before/after diff: newly broken/missing rows, with rows on
 * packages other than the update target flagged as collateral — pnpm
 * rewrites shared dependencies for the whole tree, so an update can break
 * plugins it never touched.
 */
export function diffProfileRisks(before, after, updatedName) {
  const risks = newlyBrokenOrMissing(before, after)
  return risks.map((risk) => ({
    ...risk,
    ...(risk.name !== updatedName ? { collateral: true } : {}),
  }))
}
