...
/**
 * Time a probe and capture its failure reason. Success is `{ ok: true, value }`;
 * the thrown message becomes `{ ok: false, reason }` — never throws.
 */
export async function captureProbe(run) {
  const started = Date.now()
  try {
    return { ok: true, value: await run(), elapsedMs: Date.now() - started }
  } catch (error) {
    return {
      ok: false,
      value: null,
...
/**
 * The structural `#path:...` part of a github: spec fragment (passed bare,
 * no leading `#`), or null. Pin fragments (`#<sha>` / `#<branch>`) are not
 * structural: the lockfile's codeload URLs carry the pin in the URL path,
 * never as a fragment — so commit keys derived from specs and from lockfile
 * URLs agree only when pins are dropped and the path fragment is kept.
 */
export function githubPathFragment(bare) {
  return typeof bare === 'string' && bare.startsWith('path:') && bare.length > 'path:'.length ? `#${bare}` : null
}
