...
import { classifySpec, clearScanCache, installedVersion, linkedGitState, listProfiles, npmNewest, pinnedCommits, pluginMemberNames, profileDependencyMetadata, readDeps } from './scan.js'
...
export async function updatePluginAll(name, hooks = {}, options = {}) {
  ...
  const profiles = listProfiles().filter((profile) => {
    ...
    if (!pluginMemberNames(profile, deps).has(name)) return false
