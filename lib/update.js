...
import { currentDshVersion, hostVersionsFromCore, peerWarningsForPackageDir } from './preflight.js'
...
/**
 * Peer-range warnings for one installed plugin against the running (and,
 * when the core scan knows it is behind, the target) dsh host. Disk-only
 * manifest evaluation; the core scan is cache-backed, and any failure
 * degrades to no warnings — the update itself must never be blocked by a
 * preflight hiccup.
 */
async function preflightWarnings(profile, name, spec) {
  try {
    const core = await scanCore(false)
    const versions = hostVersionsFromCore(core, currentDshVersion())
    if (versions.length === 0) return []
    const dir = packageDirOf(profileDir(profile), name, spec)
    return peerWarningsForPackageDir(dir, versions)
  } catch {
    return []
  }
}
...
export async function updatePlugin(profile, name, hooks = {}, options = {}) {
  ...
  if (kind === 'linked') {
    const linked = options.source === 'remote'
      ...
      : await updateLinkedPlugin(profile, name, spec, hooks)
    return sealIntegrity(linked, profile, name, beforeAvail)
  }
  ...
  // An explicit target (the rollback path) replaces the newest-version
  // resolution but must address exactly this package — validated, never
  // trusted client text.
  if (typeof options.target === 'string' && options.target !== '') {
    const checked = validateRollbackTarget(name, spec, options.target)
    if (checked === null) {
    ...
    repoKey = checked.repoKey
  }

  const warnings = await preflightWarnings(profile, name, spec)
  const snapshot = buildUpdateSnapshot({ profile, name, target, spec, repoKey })
  const outcome = sealIntegrity(await runPnpmUpdate(profile, name, target, { reload, targetVersion, repoKey, warnings }), profile, name, beforeAvail)
  return attachRollback(outcome, snapshot)
}
...
export async function updatePluginAll(name, hooks = {}, options = {}) {
  ...
  const rollback = items.map((i) => i.rollback).find((r) => r !== null && typeof r === 'object')
  const outcome = {
    ok: code === null,
    ...(code !== null ? { code, error } : {}),
    name,
