 ...
 * and the captured output tails; never throws for spawn failures.
 */
function runPluginAdd(file, args, viaShell) {
...
export async function updatePlugin(profile, name, hooks = {}) {
  ...
  while (attempts < UPDATE_MAX_ATTEMPTS) {
    attempts += 1
    try {
      result = await runPluginAdd(file, childArgs, viaShell)
    } catch (error) {
