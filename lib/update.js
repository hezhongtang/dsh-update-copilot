...
async function runPreflight(profile, name, spec) {
  ...
  // Breaking-change signals ride the brief's cached material — no extra
  // upstream requests; a cold or failed brief degrades to no signal.
  let breaking = []
  try {
...
export async function updatePlugin(profile, name, hooks = {}, options = {}) {
  ...
  // outcome either way.
  const preflight = await runPreflight(profile, name, spec)
