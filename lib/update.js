...
import { diffProfileRisks, snapshotAvailability } from './availability.js'
 ...
 * when the core scan knows it is behind, the target) dsh host. The core scan
 * is cache-backed; any failure degrades to no warnings — the update itself
 * must never be blocked by a preflight hiccup.
 */
async function preflightWarnings(profile, name, spec) {
  ...
  return peerWarningsForInstall({ profile, name, spec, core, dshVersion: currentDshVersion() })
}
