...
/**
 * The full preflight pass for one plugin update: peer-range warnings, the
 * hard blocker evidence (the target dsh lacks a named export the plugin
 * statically imports), upgrade-card corridor (missing edge = stop), storage
 * format boundary, and host-API capability risks. The export blocker check
 * only arms when the core is behind — a current host has no target to be
 * incompatible with — and `force` never suppresses the evidence, only the
 * refusal. Corridor / storage gaps refuse by default and honor the same
 * `force` override the caller already understands.
 */
async function runPreflight(profile, name, spec) {
  ...
  const layerFindings = buildLayerFindings({
    blockers: hard,
    warnings: [...warnings, ...capability.filter((c) => c.actionLevel !== 'informational')],
    breaking,
    corridor,
    storage,
    capability,
 ...
 * against and only peer/breaking warnings report). Results carry the shared
 * `layerFindings` vocabulary (link-time / mount-time / run-time / storage /
 * peer / breaking-card) alongside the legacy blockers/warnings/breaking lists.
 */
export async function evaluatePreflight({ name, profile = null, target = null, loadTargetExports } = {}) {
  ...
  for (const p of profiles) {
    ...
    if (spec === undefined) {
      items.push({ profile: p, decision: 'unknown', error: `${name} is not installed in profile "${p}"`, blockers: [], warnings: [], breaking: [], layerFindings: [] })
      continue
    }
    ...
    items.push({
      profile: p,
      decision: hard.length > 0 ? 'blocked' : (warnings.length > 0 || breaking.length > 0 || capability.length > 0 ? 'warning' : 'ok'),
      blockers: hard,
      warnings,
      breaking,
      capability,
