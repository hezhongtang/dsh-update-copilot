...
import { gatherTargetBlockers } from './compat.js'
import { bundleNamesOf, classifySpec, clearScanCache, githubCommitKey, inferMountRelationships, installedVersion, linkedGitState, listProfiles, npmNewest, pinnedCommits, pluginMemberNames, profileDependencyMetadata, readDeps, scanCore } from './scan.js'
import { profileDir, readJson, recordOp, repoFromRemote } from './util.js'
import { currentDshVersion, peerWarningsForInstall, breakingFindings } from './preflight.js'
...
/**
 * The full preflight pass for one plugin update: peer-range warnings plus
 * the hard blocker evidence (the target dsh lacks a named export the plugin
 * statically imports). The blocker check only arms when the core is behind —
 * a current host has no target to be incompatible with — and `force` never
 * suppresses the evidence, only the refusal.
 */
async function runPreflight(profile, name, spec) {
  ...
  const warnings = await peerWarningsForInstall({ profile, name, spec, core, dshVersion: currentDshVersion() })
  // Breaking-change signals ride the brief's cached material — the shared
  // 10-minute brief cache means no repeated upstream requests, though a
  // cold cache does fetch the brief's own material once.
  let breaking = []
  try {
    const { buildBrief } = await import('./advise.js')
    const brief = await buildBrief(name, profile)
    if (Array.isArray(brief?.breaking)) breaking = brief.breaking
  ...
  return { warnings: [...warnings, ...breaking], blockers, dshTarget }
}
 ...
 * against and only peer/breaking warnings report).
 */
export async function evaluatePreflight({ name, profile = null, target = null, loadTargetExports } = {}) {
  ...
  const coreRow = core?.packages?.[0]
  const dshTarget = typeof target === 'string' && target !== ''
    ? target
  ...
  for (const p of profiles) {
    ...
    if (spec === undefined) {
      items.push({ profile: p, decision: 'unknown', error: `${name} is not installed in profile "${p}"`, blockers: [], warnings: [], breaking: [] })
      continue
    }
    const warnings = await peerWarningsForInstall({ profile: p, name, spec, core, dshVersion: currentDshVersion() })
    let breaking = []
    try {
      const { buildBrief } = await import('./advise.js')
      const brief = await buildBrief(name, p)
      if (Array.isArray(brief?.breaking)) breaking = brief.breaking
    } catch { /* breaking signals never fail preflight */ }
    let blockers = []
    if (dshTarget !== null) {
    ...
    items.push({
      profile: p,
      decision: blockers.length > 0 ? 'blocked' : (warnings.length > 0 || breaking.length > 0 ? 'warning' : 'ok'),
      blockers,
      warnings,
      breaking,
    })
  ...
  const note = decision === 'blocked'
    ...
    : decision === 'warning'
      ...
      : decision === 'ok'
        ...
        : `${name} is not installed in any profile`
  return { name, dshTarget, decision, items, note }
}
...
/**
 * Network hardening for git remotes: abort transfers that stall (below
 * 1 KiB/s for 30s) instead of hanging until the caller's hard timeout — a
 * stalled pull becomes a fast failure the retry loop can actually spend its
 * attempts on. Harmless on local operations, which never touch these flags.
 */
const GIT_NETWORK_FLAGS = ['-c', 'http.lowSpeedLimit=1024', '-c', 'http.lowSpeedTime=30']
 ...
 * a `stash` summary.
 */
async function updateLinkedPlugin(profile, name, spec, hooks, { warnings = [] } = {}) {
  ...
  running = true
  setLiveUpdate({ name, profile, target: spec })
  try {
    ...
    const snapshot = buildUpdateSnapshot({ profile, name, target: spec, spec, repoKey: null, commit: before.commit })
    ...
    // 2. git pull, retried on transient failures only — merge conflicts need a
    //    human (git merge --abort or resolving manually), and deterministic
    //    failures (missing repo/credentials) fail fast via findFatalUpdateError.
    let pullResult = null
    let attempts = 0
    let lastFatal = null
    while (attempts < UPDATE_MAX_ATTEMPTS) {
      ...
      pullResult = await runGit(dir, [...GIT_NETWORK_FLAGS, 'pull'], UPDATE_TIMEOUT_MS)
      if (pullResult.code === 0 || mergeInProgress(dir)) break
      lastFatal = findFatalUpdateError(pullResult)
      if (lastFatal !== null || attempts >= UPDATE_MAX_ATTEMPTS) break
    ...
    return attachRollback(outcome, snapshot)
  } finally {
    running = false
    clearLiveUpdate()
  }
...
async function switchLinkedPluginToRemote(profile, name, spec, hooks) {
  ...
  running = true
  setLiveUpdate({ name, profile, target: spec, mode: 'switch' })
  try {
    const before = { spec }
    ...
    if (meta.reached === false) {
      return {
        ok: false, code: 'latest_unavailable', changed: false,
        profile, name, target: spec, before, after: before,
        attempts: 0, hotReloaded: false, requiresRestart: false,
        error: `could not resolve the newest published version of ${name} — ${meta.reason} — cannot check (not "no update")`,
        output: '',
      }
    }
    let target = null
    ...
    if (meta.newest !== null) {
      ...
      const state = await linkedGitState(dir, true)
...
export async function updatePlugin(profile, name, hooks = {}, options = {}) {
  ...
  if (preflight.blockers.length > 0 && options.force !== true) {
    const missingCount = preflight.blockers.reduce((n, finding) => n + finding.missing.length, 0)
    recordOp('warn', 'update:preflight', `${profile}/${name}: blocked — ${missingCount} named import(s) missing from target dsh ${preflight.dshTarget}`)
    return {
      ok: false,
      ...
      error: `update blocked: the target dsh ${preflight.dshTarget} does not export ${missingCount} name(s) this plugin imports — review the evidence, then pass force: true to override`,
    }
  }
  ...
  if (kind === 'linked') {
    const linked = options.source === 'remote'
      ? await switchLinkedPluginToRemote(profile, name, spec, hooks)
      : await updateLinkedPlugin(profile, name, spec, hooks, { warnings: preflight.warnings })
    const sealed = sealIntegrity(linked, profile, name, beforeAvail)
    return forced ? { ...sealed, forced: true, blockers: preflight.blockers } : sealed
  }
  ...
  if (kind === 'github') {
    ...
    if (meta.newest === null) {
      ...
      return { ok: false, code: 'latest_unavailable', error: `could not resolve the newest published version of ${name} — ${why}` }
    }
  ...
  const outcome = sealIntegrity(await runPnpmUpdate(profile, name, target, { reload, targetVersion, repoKey, warnings: preflight.warnings }), profile, name, beforeAvail)
 ...
 * switch path.
 */
async function runPnpmUpdate(profile, name, target, { reload, targetVersion = null, repoKey = null, warnings = [] }) {
  const readState = () => ({
    version: installedVersion(profile, name),
