...
import { classifySpec, clearScanCache, installedVersion, linkedGitState, npmNewest, pinnedCommits, readDeps } from './scan.js'
 ...
 * `subscribeProgress` hands a subscriber a { emit, cancel } pair; the owner
 * calls emit({ type, ... }) for every update and cancel() when done. The
 * routes layer (SSE) and the agent tool (no-op) share this bus.
 */
...
async function updateLinkedPlugin(profile, name, spec, hooks) {
  ...
  try {
    ...
    if (before.dirty) {
      sub.emit({ type: 'phase', phase: 'stash', attempt: 1, total: 1 })
    ...
    while (attempts < UPDATE_MAX_ATTEMPTS) {
      attempts += 1
      sub.emit({ type: 'phase', phase: 'pull', attempt: attempts, total: UPDATE_MAX_ATTEMPTS })
      ...
      sub.emit({ type: 'retry', attempt: attempts, total: UPDATE_MAX_ATTEMPTS })
      await sleep(retryDelay)
    }

    // 3. Restore stashed local changes.
    if (stash.stashed) {
      sub.emit({ type: 'phase', phase: 'pop', attempt: 1, total: 1 })
...
async function switchLinkedPluginToRemote(profile, name, spec, hooks) {
  ...
  try {
    ...
    const outcome = await runPnpmUpdate(profile, name, target, { reload: typeof hooks.reload === 'function' ? hooks.reload : null, sub, targetVersion: channel === 'npm' ? target.split('@').pop() : null })
...
export async function updatePlugin(profile, name, hooks = {}, options = {}) {
  ...
  return runPnpmUpdate(profile, name, target, { reload, sub, targetVersion, repoKey })
}
 ...
 * switch path.
 */
async function runPnpmUpdate(profile, name, target, { reload, sub, targetVersion = null, repoKey = null }) {
  const readState = () => ({
  ...
  while (attempts < UPDATE_MAX_ATTEMPTS) {
    attempts += 1
    sub.emit({ type: 'phase', phase: 'start', attempt: attempts, total: UPDATE_MAX_ATTEMPTS })
    try {
      result = await runPluginAdd(file, childArgs, viaShell, (line) => {
        const parsed = parseProgressLine(line)
        if (parsed !== null) {
          sub.emit({ type: 'progress', ...parsed, attempt: attempts, total: UPDATE_MAX_ATTEMPTS })
        } else {
          sub.emit({ type: 'line', text: line, attempt: attempts, total: UPDATE_MAX_ATTEMPTS })
        }
      })
    } catch (error) {
