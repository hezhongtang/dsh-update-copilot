...
import { attachRisks, packageDirOf, snapshotAvailability } from './availability.js'
...
function captureAvailability(profile, name) {
  try {
    const dir = profileDir(profile)
    const manifest = readJson(join(dir, 'package.json'))
    return snapshotAvailability(profile, [name], {
      profileDir: dir,
      deps: readDeps(profile),
...
function sealIntegrity(outcome, profile, name, before) {
  ...
  return attachRisks(outcome, before, captureAvailability(profile, name))
}
...
export async function updatePlugin(profile, name, hooks = {}, options = {}) {
  ...
  const beforeAvail = captureAvailability(profile, name)
  ...
  return sealIntegrity(await runPnpmUpdate(profile, name, target, { reload, targetVersion, repoKey, warnings }), profile, name, beforeAvail)
}
