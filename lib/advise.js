 ...
 */
import { execText, fetchJson, npmPageOf, repoFromRemote, repoOf, repoUrlOf } from './util.js'
import { listProfiles, readDeps, scanCore, scanProfile } from './scan.js'
import { peerWarningsForInstall, breakingFindings } from './preflight.js'
...
async function linkedChannelBrief(dir, current, latest) {
  ...
  if (count !== null) {
    ...
    brief.commits = (log ?? '').split('\n').filter(Boolean).map((line) => ({
      ...
      sha: line.slice(0, 8),
      url: repo !== null ? `https://github.com/${repo}/commit/${line.slice(0, 8)}` : null,
    }))
...
async function buildProfileBrief(profile, name, force = false) {
  ...
  const key = `${profile}/${name}/${String(row.current)}..${String(row.latest)}/${row.availability?.state ?? ''}`
  ...
  // Breaking-change signals ride the material this brief already fetched —
  // no extra upstream requests; absent material means no signal.
  const breaking = breakingFindings({ releases: material.releases ?? [], commits: material.commits ?? [] })
  const brief = {
    profile,
    name,
    kind: row.kind,
  ...
  if (row.availability !== undefined) {
    brief.availability = row.availability
    const state = row.availability.state
    if (state === 'broken' || state === 'missing' || state === 'disabled') {
      brief.availabilityNote = `${state}: ${row.availability.reasons}`
      brief.recommendation = `${brief.availabilityNote}. ${brief.recommendation}`
    }
  }
  // Peer-range warnings against the running (and, when the core is behind,
  // the target) dsh host; degrades to no warnings on any failure.
  try {
    const core = await scanCore(force)
    const warnings = await peerWarningsForInstall({ profile, name, spec, core })
    if (warnings.length > 0) brief.warnings = warnings
  } catch { /* preflight warnings never fail the brief */ }
