 ...
 */
import { execText, fetchJson, npmPageOf, profileDir, repoFromRemote, repoOf, repoUrlOf } from './util.js'
...
async function buildProfileBrief(profile, name, force = false) {
  ...
  // the target) dsh host. Disk-only evaluation over the installed manifest;
  // any failure degrades to no warnings.
  try {
    ...
    if (versions.length > 0) {
      ...
      const warnings = peerWarningsForPackageDir(dir, versions)
