 ...
 * brief names carries a clickable URL (npm version pages, GitHub commits,
...
import { fetchJson, repoFromRemote, repoOf } from './util.js'
...
async function npmChannelBrief(name, current, latest) {
  ...
  brief.repoUrl = repository !== null ? `https://github.com/${repository}` : null
...
export async function buildBrief(profile, name, force = false) {
  ...
  const brief = {
    ...
    repoUrl: material.repoUrl ?? (row.repo ? `https://github.com/${row.repo}` : null),
    material,
